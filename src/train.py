...
def write_error_analysis(test_df, output_path):
    """Write aggregate error analysis without publishing prompt text."""
    rows = []
    for actual, predicted, label in [(1, 0, "False negatives"), (0, 1, "False positives")]:
        subset = test_df[(test_df["label"] == actual) & (test_df["pred"] == predicted)]
        lengths = subset["prompt"].astype(str).str.len()
        rows.append({
            "label": label,
            "count": int(len(subset)),
            "median_chars": float(lengths.median()) if len(subset) else 0.0,
            "mean_score": float(subset["score"].mean()) if len(subset) else 0.0,
        })

    total = len(test_df)
    errors = int((test_df["label"] != test_df["pred"]).sum())
    text = [
        "# Error analysis",
        "",
        ...
        "## Summary",
        "",
        f"- Held-out examples: {total}",
        f"- Misclassified examples: {errors}",
        f"- Error rate: {errors / total:.3f}" if total else "- Error rate: n/a",
        "",
    ...
    for row in rows:
        text.append(
            f"| {row['label']} | {row['count']} | {row['median_chars']:.1f} | {row['mean_score']:.3f} |"
        )
    text += [
        "",
        "## Interpretation",
...
def main():
    ...
    write_error_analysis(test_df, ROOT / "reports" / "error_analysis.md")
    print(f"\n{len(errors)} misclassified test examples summarized in reports/error_analysis.md")
    print("Raw prompt text was not written to the report.")
